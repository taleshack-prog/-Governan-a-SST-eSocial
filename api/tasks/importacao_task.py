...
Para LTCAT inclua em dados: empresa, cnpj, data_emissao (YYYY-MM-DD), responsavel_tecnico(nome,registro,conselho), agentes_nocivos(codigo_tabela24,descricao,setor,nivel_exposicao,unidade_medida,epc_eficaz,epi_eficaz,epi_ca)
...
def salvar_ltcat(db, dados, empresa_id, nome_arquivo, conteudo=None, texto_completo=""):
    ...
    doc = DocumentoTecnico(
        ...
        status="ativo", metadata_doc={"cnpj": dados.get("cnpj")},
    )
    db.add(doc)
    db.flush()
    ...
    for ag in dados.get("agentes_nocivos", []):
        ...
        for trab_id in (trab_ids or [None]):
            db.add(AgenteNocivo(
                id=uuid.uuid4(), empresa_id=empresa_id, trabalhador_id=trab_id,
                documento_origem_id=doc.id, codigo_tabela24=codigo, descricao_agente=descricao,
                nivel_exposicao=str(ag.get("nivel_exposicao","") or "")[:100] or None,
                unidade_medida=str(ag.get("unidade_medida","") or "")[:30] or None,
                epc_eficaz=ag.get("epc_eficaz"), epi_eficaz=ag.get("epi_eficaz"),
                epi_ca=str(ag.get("epi_ca","") or "")[:20] or None,
                data_inicio=parse_data(dados.get("data_emissao")) or date.today(),
                created_by_ai=True, confidence_score=0.85, needs_review=True, trecho_original=descricao,
            ))
            agentes_criados += 1
...
def salvar_trabalhadores(db, dados, empresa_id):
    ...
    for t in dados.get("trabalhadores", []):
        ...
        if cpf:
            if db.execute(select(Trabalhador).where(Trabalhador.empresa_id==empresa_id, Trabalhador.cpf==cpf)).scalar_one_or_none():
                duplicados += 1
                continue
...
def salvar_cat(db, dados, empresa_id):
    ...
    if cpf:
        trab = db.execute(select(Trabalhador).where(Trabalhador.empresa_id==empresa_id, Trabalhador.cpf==cpf)).scalar_one_or_none()
    cat = CatRegistro(
